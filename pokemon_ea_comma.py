...
def run_ea(
    ...
    n_offspring       : λ  – children produced per generation.
                        Must satisfy λ ≥ µ: only offspring compete for
                        survival; parents are fully discarded each generation.
    ...
    """
    if n_offspring < pop_size:
        raise ValueError(
            f"µ,λ selection requires λ ≥ µ, but got λ={n_offspring} < µ={pop_size}. "
            f"Increase n_offspring or decrease pop_size."
        )
    ...
    _print(f"{'='*60}")
    _print(f"  µ={pop_size}  λ={n_offspring}  generations={n_generations}  (µ,λ selection)")
    ...
    for gen in range(1, n_generations + 1):
        ...
        # Parents are entirely discarded; next generation is chosen
        # solely from the λ offspring, ranked by fitness.
        ranked = sorted(
            zip(offspring, off_fits),
            key=lambda x: x[1],
            reverse=True,
        )
        population = [g for g, _ in ranked[:pop_size]]
        fitnesses  = [f for _, f in ranked[:pop_size]]
...
def visualize_fitness_landscape(
    history:          dict,
    final_population: list[list[str]],
    final_fitnesses:  list[float],
    best_genome:      list[str],
    n_opponents:      int   = 6,
    n_battles:        int   = 3,
    save_path:        str   = "fitness_landscape.png",
) -> None:
    """
    Four-panel fitness landscape visualization for the permutation EA.

    Panel 1 — Convergence Curve
        Best and average fitness over every generation, with a ±1 std band
        around the average.  Shows whether the population is converging or
        still exploring.

    Panel 2 — Local Swap-Neighbour Landscape (6×6 heatmap)
        For the best genome found, evaluate every possible single-swap
        mutation (C(6,2)=15 distinct neighbours).  Each cell [i,j] shows
        Δfitness = neighbour_fitness − best_fitness.  Negative = the swap
        hurts; positive = a better neighbour exists.  This is the literal
        local fitness landscape around the optimum: if all cells are ≤ 0
        the solution is a local optimum w.r.t. single swaps.

    Panel 3 — Population Scatter (2-D MDS projection)
        Pairwise distances between genomes are computed using the Kendall-tau
        distance on a species-presence/order encoding, then projected to 2-D
        with classical multidimensional scaling (MDS).  Points are coloured
        by fitness, size reflects rank.  Shows whether the population has
        converged to a tight cluster or still spans diverse regions of the
        search space.

    Panel 4 — Species-Slot Frequency Heatmap
        Across all individuals in the final population, count how often each
        species appears in each of the 6 team slots.  Reveals which slots
        the EA has converged on (dark column = slot locked in) and which
        species are reliably placed at specific positions.

    Parameters
    ----------
    history          : dict returned by run_ea (keys: best, avg, std)
    final_population : list of genomes in the last generation
    final_fitnesses  : corresponding fitness values
    best_genome      : the single best genome found
    n_opponents      : opponents for neighbour fitness evaluation
    n_battles        : battles per opponent for neighbour evaluation
    ...
    fig.suptitle(
        "Pokémon Gen-1 EA — Fitness Landscape Analysis",
    ...
    ax2 = fig.add_subplot(gs[0, 1])   # swap-neighbour heatmap
    ax3 = fig.add_subplot(gs[1, 0])   # MDS population scatter
    ax4 = fig.add_subplot(gs[1, 1])   # species-slot frequency

    # ── PANEL 1: Convergence curve ────────────────────────────
    gens  = np.arange(len(history["best"]))
    best  = np.array(history["best"])
    avg   = np.array(history["avg"])
    std   = np.array(history["std"])

    ax1.fill_between(gens, avg - std, avg + std,
                     alpha=0.20, color=BLUE, label="_nolegend_")
    ax1.plot(gens, avg,  color=BLUE,   linewidth=1.8, label="Avg fitness",   zorder=3)
    ax1.plot(gens, best, color=ACCENT, linewidth=2.2, label="Best fitness",  zorder=4)
    ax1.scatter(gens[-1], best[-1], color=ACCENT, s=80, zorder=5)
    ax1.axhline(2.0, color=RED, linewidth=0.8, linestyle="--", alpha=0.5, label="Max (2.0)")

    ax1.set_title("Convergence Curve", color=WHITE, fontsize=11, pad=8)
    ax1.set_xlabel("Generation")
    ax1.set_ylabel("Fitness  (win rate + HP ratio)")
    ax1.set_ylim(0, 2.1)
    ax1.legend(fontsize=8, facecolor="#1a1a2e", edgecolor=GRAY)
    ax1.grid(True, alpha=0.3)

    # ── PANEL 2: Local swap-neighbour landscape ───────────────
    print("  [Viz] Evaluating local swap neighbourhood (15 pairs) …", flush=True)
    best_fit = evaluate_fitness(best_genome, n_opponents, n_battles)
    n = len(best_genome)

    delta_matrix = np.full((n, n), np.nan)
    swap_pairs   = list(combinations(range(n), 2))

    for i, j in swap_pairs:
        nbr     = best_genome.copy()
        nbr[i], nbr[j] = nbr[j], nbr[i]
        nbr_fit = evaluate_fitness(nbr, n_opponents, n_battles)
        delta_matrix[i, j] = nbr_fit - best_fit
        delta_matrix[j, i] = nbr_fit - best_fit   # symmetric

    slot_labels = [f"Slot {k+1}\n{species_names.get(best_genome[k], best_genome[k])[:9]}"
                   for k in range(n)]

    # Centre-diverging colormap: green = better, red = worse
    max_abs = max(np.nanmax(np.abs(delta_matrix)), 0.01)
    norm    = mcolors.TwoSlopeNorm(vmin=-max_abs, vcenter=0, vmax=max_abs)
    cmap    = plt.cm.RdYlGn

    masked = np.ma.masked_invalid(delta_matrix)
    im2    = ax2.imshow(masked, cmap=cmap, norm=norm, aspect="auto")

    ax2.set_xticks(range(n)); ax2.set_xticklabels(slot_labels, fontsize=7)
    ax2.set_yticks(range(n)); ax2.set_yticklabels(slot_labels, fontsize=7)

    # Annotate each cell with the Δ value
    for i in range(n):
        for j in range(n):
    ...
    cb2.set_label("Δ Fitness vs best", color=WHITE, fontsize=8)
    cb2.ax.yaxis.set_tick_params(color=WHITE)

    any_pos = np.any(~np.isnan(delta_matrix) & (delta_matrix > 0))
    subtitle = "(green = better swap exists)" if any_pos else "(local optimum — no improving swap)"
    ax2.set_title(f"Local Swap-Neighbour Landscape\n{subtitle}",
                  color=WHITE, fontsize=11, pad=8)

    # ── PANEL 3: Population scatter — 2-D MDS ────────────────
    pop    = final_population
    fits   = np.array(final_fitnesses)
    m      = len(pop)

    # Distance metric: symmetric Kendall-tau on position rankings.
    # Encode each genome as a dict {species: slot_index} then compare
    # shared species by position difference.
    def genome_distance(g1: list[str], g2: list[str]) -> float:
        """
        Generalised Kendall-tau distance for non-identical-element permutations.
        For species present in both teams, count position disagreements.
        For species unique to one team, add a fixed penalty per missing species.
        """
        pos1 = {s: i for i, s in enumerate(g1)}
        pos2 = {s: i for i, s in enumerate(g2)}
        shared = set(g1) & set(g2)
        uniq   = len(set(g1) ^ set(g2))   # symmetric difference

        # Order disagreements among shared species
        disagree = 0
        shared_l = list(shared)
        for a in range(len(shared_l)):
            for b in range(a + 1, len(shared_l)):
                sa, sb = shared_l[a], shared_l[b]
                if (pos1[sa] - pos1[sb]) * (pos2[sa] - pos2[sb]) < 0:
                    disagree += 1

        return disagree + uniq * 1.5   # weight unshared species heavily

    dist_mat = np.zeros((m, m))
    for a in range(m):
        for b in range(a + 1, m):
            d = genome_distance(pop[a], pop[b])
            dist_mat[a, b] = d
            dist_mat[b, a] = d

    # Classical MDS: eigendecomposition of the double-centred distance² matrix
    D2 = dist_mat ** 2
    n_pts = D2.shape[0]
    ...
    # Keep first 2 positive components
    pos_mask = eigvals > 0
    coords   = eigvecs[:, :2] * np.sqrt(np.maximum(eigvals[:2], 0))

    # Scatter, coloured by fitness
    sc3 = ax3.scatter(
        coords[:, 0], coords[:, 1],
        c=fits, cmap="plasma", vmin=fits.min(), vmax=fits.max(),
        s=90 + 120 * (fits - fits.min()) / max(fits.max() - fits.min(), 1e-9),
        edgecolors=WHITE, linewidths=0.5, zorder=4,
    )
    # Mark the best individual
    best_idx = int(np.argmax(fits))
    ax3.scatter(coords[best_idx, 0], coords[best_idx, 1],
                marker="*", s=320, color=ACCENT, zorder=5, label="Best")

    cb3 = fig.colorbar(sc3, ax=ax3, fraction=0.046, pad=0.04)
    cb3.set_label("Fitness", color=WHITE, fontsize=8)
    cb3.ax.yaxis.set_tick_params(color=WHITE)

    ax3.set_title("Population Scatter — 2-D MDS (Kendall-tau distance)",
                  color=WHITE, fontsize=11, pad=8)
    ax3.set_xlabel("MDS dim 1")
    ax3.set_ylabel("MDS dim 2")
    ax3.legend(fontsize=8, facecolor="#1a1a2e", edgecolor=GRAY)
    ax3.grid(True, alpha=0.3)

    # ── PANEL 4: Species-slot frequency heatmap ───────────────
    # Collect only species that appear in at least one genome of the final pop
    active_species = sorted({s for g in pop for s in g})

    freq = np.zeros((len(active_species), 6))
    for g in pop:
        for slot, sid in enumerate(g):
            row = active_species.index(sid)
            freq[row, slot] += 1
    freq /= max(len(pop), 1)   # normalise to [0, 1]

    # Sort rows by total frequency (most-used species at top)
    row_order = np.argsort(-freq.sum(axis=1))
    freq      = freq[row_order]
    sp_labels = [species_names.get(active_species[i], active_species[i])
                 for i in row_order]

    im4 = ax4.imshow(freq, cmap="YlOrRd", vmin=0, vmax=1, aspect="auto")
    ax4.set_xticks(range(6))
    ax4.set_xticklabels([f"Slot {k+1}" for k in range(6)], fontsize=8)
    ax4.set_yticks(range(len(sp_labels)))
    ax4.set_yticklabels(sp_labels, fontsize=7)

    # Annotate non-zero cells
    for r in range(freq.shape[0]):
        for c in range(6):
            if freq[r, c] > 0.05:
                txt_col = "black" if freq[r, c] > 0.55 else WHITE
                ax4.text(c, r, f"{freq[r, c]:.0%}", ha="center", va="center",
                         fontsize=7, color=txt_col, fontweight="bold")

    cb4 = fig.colorbar(im4, ax=ax4, fraction=0.046, pad=0.04)
    cb4.set_label("Slot occupancy rate", color=WHITE, fontsize=8)
    cb4.ax.yaxis.set_tick_params(color=WHITE)

    ax4.set_title("Species-Slot Frequency (final population)",
                  color=WHITE, fontsize=11, pad=8)

    # ── Save ──────────────────────────────────────────────────
    fig.savefig(save_path, dpi=140, bbox_inches="tight", facecolor=BG)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    # ── Run the EA ────────────────────────────────────────────
    best_genome, best_fitness, history, final_pop, final_fits = run_ea(
        pop_size          = 20,
        n_offspring       = 40,   # λ must be ≥ µ for µ,λ selection
        n_generations     = 30,
        tournament_size   = 3,
        mutation_prob     = 0.50,
        n_opponents       = 8,
    ...
    print("Done. See fitness_landscape.png")
